          ...
          <li class="user-input-checkbox">
            <label class="checkbox-container checkbox-container-small">
              <input
                :id="'address' + index"
                type="checkbox"
                @click="unselectAllAddresses('address' + index)">
...
export default {
  ...
  mounted() {
    ...
    this.$refs.switchAddressModal.$once('shown', () => {
      ...
      this.getAddresses(5, baseIndex[1]).then(_result => {
  ...
  methods: {
    ...
    unlockWallet() {
      ...
      this.$store.state.wallet.wallet.setActiveAddress(
        this.tenativeSwitchedAddress.address,
        this.tenativeSwitchedAddress.index
      );
      this.$refs.switchAddressModal.hide();
      // this.$store.dispatch('decryptWallet', this.hardwareWallet)
      // this.$router.push({path: 'interface'})
    },
    setAddress(details, identify) {
      this.unselectAllAddresses(identify);
      this.tenativeSwitchedAddress = {
    ...
    priorAddressSet() {
      ...
      if (priorSetStart < this.minIndex) {
        ...
        this.getAddresses(this.count, priorSetStart).then(addressSet => {
